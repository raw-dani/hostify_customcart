{if $checkout}
    ...
    {include file="orderforms/hostify_customcart/checkout.tpl"}
...
{else}
    ...
    {include file="orderforms/hostify_customcart/hostify-common.tpl"}
    ...
    <div id="order-standard_cart">
        ...
        <div class="row">
            <div class="cart-sidebar">
                ...
                {include file="orderforms/hostify_customcart/hostify-sidebar-categories.tpl"}
            ...
            <div class="cart-body">
                ...
                {include file="orderforms/hostify_customcart/hostify-sidebar-categories-collapsed.tpl"}
                                    ...
                                    <div class="item">
                                        <div class="row">
                                            <div class="{if $showqtyoptions}col-sm-5{else}col-sm-7{/if}">
                                                ...
                                                {* variabel custom - inisialisasi dengan nilai default *}
                                                {assign var="percentage" value=$diskonpromo|default:30}
                                                {assign var="lisensi" value=$maxusepromo|default:20}
                                                {assign var="idproduk" value=2}

                                                {* Inisialisasi variabel global untuk kalkulasi *}
                                                {if !$totalwithdiskonx}{assign var="totalwithdiskonx" value=0}{/if}
                                                {if !$totalx}{assign var="totalx" value=0}{/if}
                                                    ...
                                                    {if $product.pid == $idproduk}
                                                        <small>
                                                            {foreach key=confnum item=configoption from=$product.configoptions}
                                                                {* variabel custom dengan error checking *}
                                                                {assign var="harga" value=0}
                                                                {if isset($configoption.option) && $configoption.option}
                                                                    {$prices = explode(" ",$configoption.option)}
                                                                    {if isset($prices[2])}
                                                                        {$d = explode(",",$prices[2])}
                                                                        {if isset($d[0]) && isset($d[1]) && isset($d[2])}
                                                                            {assign var="depan" value=$d[0]|default:0}
                                                                            {assign var="belakang" value=$d[1]|default:0}
                                                                            {assign var="belakang2" value=$d[2]|default:0}
                                                                            {$harga = "$depan$belakang$belakang2"|intval}
                                                                        {/if}
                                                                    {/if}
                                                                {/if}

                                                                {* Pastikan harga valid sebelum kalkulasi *}
                                                                {if $harga > 0 && $percentage > 0 && $percentage <= 100}
                                                                    {math assign="persen" equation='x/y*h' y=100 x=$percentage h=$harga format="%.2f"}
                                                                    {math assign="hargadasar" equation='y-x' y=$harga x=$persen format="%.2f"}

                                                                    {assign var="config_qty" value=$configoption.qty|default:0|intval}
                                                                    {math assign="qtynondiskon" equation='y-x' y=$config_qty x=$lisensi format="%.0f"}

                                                                    {math assign="subtotdiskon" equation='y*x' y=$hargadasar x=$lisensi format="%.2f"}
                                                                    {math assign="subtotnondiskon" equation='y*x' y=$harga x=$qtynondiskon format="%.2f"}

                                                                    {math assign="totalx" equation='y+x' y=$subtotdiskon x=$subtotnondiskon format="%.2f"}
                                                                    {math assign="totalwithdiskonx" equation='y*x' y=$config_qty x=$hargadasar format="%.2f"}

                                                                    {if $config_qty <= $lisensi}
                                                                        &nbsp;&raquo; {$configoption.name}: {if $configoption.type eq 1 || $configoption.type eq 2}{$configoption.option}{elseif $configoption.type eq 3}{if $configoption.qty}{$configoption.option}{else}{$LANG.no}{/if}{elseif $configoption.type eq 4}{$config_qty} x {$hargadasar}{/if}<br />
                                                                    {else}
                                                                        &nbsp;&raquo; {$configoption.name}: {if $configoption.type eq 1 || $configoption.type eq 2}{$configoption.option}{elseif $configoption.type eq 3}{if $configoption.qty}{$configoption.option}{else}{$LANG.no}{/if}{elseif $configoption.type eq 4} Discount {$lisensi} x {$hargadasar} , Normal {$qtynondiskon} x {$harga}{/if}<br />
                                                                    {/if}
                                                                {else}
                                                                    &nbsp;&raquo; {$configoption.name}: {if $configoption.type eq 1 || $configoption.type eq 2}{$configoption.option}{elseif $configoption.type eq 3}{if $configoption.qty}{$configoption.option}{else}{$LANG.no}{/if}{elseif $configoption.type eq 4}{$configoption.qty|default:0} x {$configoption.option}{/if}<br />
                                                                {/if}
                                                            {/foreach}
                                                        </small>
                                                    {else}
                                ...
                                {foreach $domains as $num => $domain}
                                    <div class="item">
                                        <div class="row">
                                            ...
                                            <div class="col-sm-4 item-price">
                                                {assign var="pharga" value=0}
                                                {if isset($domain.price) && $domain.price}
                                                    {$pprices = explode(" ",$domain.price)}
                                                    {if isset($pprices[1])}
                                                        {$pd = explode(",",$pprices[1])}
                                                        {if isset($pd[0]) && isset($pd[1]) && isset($pd[2])}
                                                            {assign var="pdepan" value=$pd[0]|default:0}
                                                            {assign var="pbelakang" value=$pd[1]|default:0}
                                                            {assign var="pbelakang2" value=$pd[2]|default:0}
                                                            {$pharga = "$pdepan$pbelakang$pbelakang2"|intval}
                                                        {/if}
                                                    {/if}
                                                {/if}

                                                {* Akumulasi total dengan error checking *}
                                                {if $pharga > 0}
                                                    {assign var="totalwithdiskonx" value=$totalwithdiskonx|default:0}
                                                    {assign var="totalx" value=$totalx|default:0}
                                                    {$totalwithdiskonx = $totalwithdiskonx + $pharga}
                                                    {$totalx = $totalx + $pharga}
                                                {/if}
                            ...
                            <div class="summary-container">
                                ...
                                {if $promotioncode || $taxrate || $taxrate2}
                                    <div class="bordered-totals">
                                        ...
                                        {if $taxrate}
                                            {if $product.pid == $idproduk}
                                                {assign var="current_total" value=$vartotalsummary|default:0}
                                                {if $current_total > 0}
                                                    {math assign="taxpersen" equation='x/y*h' y=100 x=$taxrate h=$current_total format="%.2f"}
                                                    {math assign="totalday" equation='x+y' y=$taxpersen x=$current_total format="%.2f"}
                                                {else}
                                                    {assign var="taxpersen" value=0}
                                                    {assign var="totalday" value=0}
                                                {/if}
                                                <div class="clearfix">
